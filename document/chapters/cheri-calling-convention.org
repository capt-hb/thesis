* CHERI-MIPS Calling Convention
  <<sec:cheri-cc>>
  This section describes the calling convention used in CHERI-MIPS (for the purecap ABI\todo{never introduced this concept}), which
...
** Invoking A Function
   ...
   is done by using the following instruction sequence:\todo{if you show code, some more explanation is required (maybe this is work in progress?)}
   ...
   consequently jumping to a function will always look like ~cljar $c12, $c17~\todo{cjalr? typo? + isnt the current return address stored in c17, rather than the one you will be using in the next frame? This is just a guess, but I'd check.}.
