...
*** Sealed Capabilities
    ...
    in $ct$, more specifically, the 24 least significant bits of the cursor of $ct$\todo{I think it needs to have a set seal permission set as well; Id look into this and mention it}.
    This cursor is not allowed to be greater than the maximum otype (which is
    $2^24 - 1 - 16$\todo{typesetting}, the $- 16$ subtracts the reserved otypes that have a special meaning
    ...
    not have the execute permission set. $cs$\todo{dont start sentences in mathematical symbols -> The value $cs$ ...} will be placed in the program counter capability register (PCC)
