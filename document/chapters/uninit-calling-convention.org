* Secure Calling Convention 
  ...
  properties. These are defined as\todo{are these literal quotes? If not,I wouldnt indent.}:
  #+begin_quote
  Well-bracketed control flow (WBCF) expresses that invoked functions must either return to their
  callers, invoke other functions themselves\todo{in WB fashion} or diverge, and generally holds in programming
  ...
  function returns \parencite{skorstengaard2019stktokens}.\remark{What about when a function returns to another function? Definition probably depends on the use case.}
  #+end_quote
 \todo{also, why did you not introduce these concepts with Lau's calling convention? then you could just contrast the difference with the stack clearing here}
  
  To achieve WBCF, the calling convention prevents adversaries from jumping to return capabilities
  that the adversary might have stored on the stack, this is described in Figure \ref{fig:secure-cc-1}\todo{referring to a figure shouldnt replace the explanation here + the 'secret' in the figure is a bit misleading, as it suggests a confidentiality-related use case, rather than a WBCF one}.
  ...
  attempt to supply a different stack capability or return capability.\todo{this paragraph is too vague to me; high-level and details mixed in - either say this later or clarify a bit - why sealing? what is the conceptual role of each (or dont mention return and stack specifically yet)? code-data pair?}
  ...
  having to clear the unused part of the stack. The requirement to clear the unused part of the stack
  before returning to the caller of the function is also no longer required\todo{bit of context/explanation here? also, Lau already mentions this in his journal version iirc, so you can reference it, as it is not a novel idea}.
  ...
  stack capability with the *UL*\todo{URWL?} permissions. Figure \ref{fig:uninit-stack} shows the concept
...
** Invoking A Function
   ...
   One important caveat is that stack frames should begin at addresses that are multiples of 32\todo{why?},
   ...
   - the stack capability\todo{clarify: not the same copy as in point 1} needs to be shrunk to no longer include the stack frame of the caller
   ...
   - the return capability is constructed as the current program counter capability but
     incremented so that it returns to the instruction that restores the old stack capability\remark{do you require explicit instructions to restore the old stack capability, if it can be passed as the data capability to the next stack frame and then be unsealed upon return by ccall? maybe Im missing something relating to index\permissions -- Ok, this got cleared up below, maybe reformulate this a bit here to clarify},
   ...
   \todo[inline]{You do not explicitate whether or not the 2 capabilities in your sealed pair should be local or not - I think it is important to clarify this here. Can you have a non-local code capability? Does CHERI allow this? What would be the consequences if it did; could this work? }
   ...
   will always be 28\todo{how did you obtain 28?}.
...
** Returning From A Function
   ...
   provided. The selector\todo{briefly repeat what this is?} used for /CCall/ is 1, meaning that the return capability is unsealed and placed
   ...
   Before jumping to the return capability, the non-return registers need to be cleared.\todo{This is the epilogue; merge the sections if you want to mention it here.}
   ...
   the number of registers that may be used with the ccall instruction\todo{I wouldve appreciated this exaplanation during the itemization above, I was wondering about this}, in order to avoid the need
...
** Function Prologue
   ...
   \todo{Wat gebeurt er wanneer de stack pointer die we binnenkrijgen een normale, global pointer is en niet RWLX? Kan je dan ge-confused-deputy-d worden? Heb je prepstk van Lau nodig of niet?}
