...
** Invoking A Function
   ...
   On lines 5-7 the return capability is created and also sealed using the same unique seal as for
   ...
   is 28 in case no arguments need to be stored in registers or spilled to the stack.
